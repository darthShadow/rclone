...
func (d *Dir) _age(when time.Time) (age time.Duration, stale bool) {
	...
	if !stale && d.vfs.Opt.PollInterval != 0 {
		...
		if polledAge > d.vfs.Opt.PollInterval {
...
func (d *Dir) readActualModTime() (modTime time.Time, cephRcTime time.Time) {
	modTime, cephRcTime = time.Time{}, time.Time{}

	dirPath := join.FilePathJoin(d.f.Root(), d.path)

	// TODO: Check if this is the right way to get the modtime
...
func (d *Dir) hasModTimeChanged() bool {
	...
	dirModTime, dirCephRcTime := d.readActualModTime()
	if dirModTime.IsZero() {
		fs.Debugf(d.path, "failed to get modtime, skipping modtime check")
		return true
	}

	fs.Debugf(d.path, "read ceph.dir.rctime: %v, modtime: %v", dirCephRcTime, dirModTime)

	dirModTimeChanged, dirCephRcTimeChanged := false, false

	if !dirCephRcTime.IsZero() && !d.polledCephRcTime.Equal(dirCephRcTime) {
		d.polledCephRcTime = dirCephRcTime
		dirCephRcTimeChanged = true
	}

	if !d.polledModTime.Equal(dirModTime) {
		d.polledModTime = dirModTime
		dirModTimeChanged = true
	}

	return dirModTimeChanged || dirCephRcTimeChanged
}
